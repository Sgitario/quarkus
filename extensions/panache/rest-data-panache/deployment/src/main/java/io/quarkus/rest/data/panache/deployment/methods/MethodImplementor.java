...
public abstract class MethodImplementor {
    ...
    protected MethodImplementor(boolean isResteasyClassic, boolean isReactivePanache) {
    ...
    protected void addProducesJsonAnnotation(AnnotatedElement element, boolean isHal) {
        if (isHal) {
    ...
    protected String appendToPath(String... parts) {
        for (int index = 0; index < parts.length; index++) {
            if (parts[index].endsWith("/")) {
                parts[index] = parts[index].substring(0, parts[index].lastIndexOf("/"));
            }
        }
